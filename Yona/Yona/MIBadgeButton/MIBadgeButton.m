...
- (void) setupBadgeViewWithString:(NSString *)string
{
    ...
    [badgeLabel setFrame:CGRectMake(self.bounds.size.width - 10 + horizontal, -(badgeSize.height / 2) - 10 + vertical, badgeSize.width,  badgeSize.width > maxWidth ? badgeSize.height : badgeSize.width)];
    [self setupBadgeStyle];
...
- (void) setupBadgeStyle
{
    ...
    badgeLabel.layer.cornerRadius = badgeLabel.bounds.size.width > maxWidth ? 8 : badgeLabel.bounds.size.width / 2;
}
