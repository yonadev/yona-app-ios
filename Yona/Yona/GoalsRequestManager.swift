...
//                }
                ...
                self.getUserGoals(activities!){ (success, serverMessage, serverCode, nil, goals, error) in
                    if let goals = goals {
                        let goalsTypeSorted = self.sortGoalsIntoArray(goalType, goals: goals)
                        onCompletion(success, error?.userInfo[NSLocalizedDescriptionKey] as? String, self.APIService.determineErrorCode(error), nil, goalsTypeSorted, error)
                    } else {
...
//                }
    ...
    func deleteUserGoal(goalEditLink: String?, onCompletion: APIResponse) {
        ...
        self.goalsHelper(httpMethods.delete, body: nil, goalLinkAction: goalEditLink) { (success, message, serverCode, goal, goals, error) in
            if success {
                ActivitiesRequestManager.sharedInstance.getActivitiesArray({ (success, message, code, activites, error) in
                    self.getUserGoals(activites!) { (success, message, code, nil, goals, error) in
